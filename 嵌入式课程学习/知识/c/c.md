...
- gcc 常用选项
  ...
  - | 选项          | 用法               | 含义                                                         | 工具   | 注意事项                                                     |
  	...
  	| -static       |                    | 生成静态链接（静态库）                                       |        | 生成静态可执行文件（体积大，但不需要依赖 dll）。gcc默认是生成动态的可执行文件 |
  ...
  - 工作流程
    ...
    - 都有的信息
      ...
      - ```powershell
        ...
        # Target: x86_64-w64-mingw32   x86的cpu架构 - 表示 Windows 64-bit（w64 是 MinGW-w64 项目特有的标记） - 表示 MinGW 环境（即使是 64 位也保留 mingw32 这个名字，这是历史惯例）
        # 目标：架构/位数 - 厂商(有些书写会省略) -  操作系统种类
...
- 共用体里的成员：共用同一块内存。**共用体占用空间大小为最大的成员变量所需的空间**（也会像结构体一样进行总体对齐）
...
- ![](../../img/day05-C中关键字.png)
  ...
  - **static**：
    ...
    - **修饰函数、全局变量，让其只能在本文中使用**
    	...
    	- Linux内核中使用统一的函数路由表，需要添加`static`，来保证在全局的命名空间不会被污染。如果需要将内核驱动模块的API暴露，使用`EXPORT_SYMBOL_GPL()`显示导出符号表。
...
- 字符串的赋值
  ...
  - sizeof(str);
  	...
  	- str为字符串：字符串的大小（包括‘\0’）。          字符串长度（不包括‘\0’）
  		- `sizof("abc"); `/  char str[] = “abc”; sizeof(str); : 包含了‘\0’的字符串大小
  		- `char *str = "abc"; sizof(str);` : 只是指针大小
...
- 函数指针：`函数返回值类型 (*指针变量名)(函数形参的数据类型) = 函数名;`
  ...
  - 回调函数：先把函数注册在这里，你先去干别的，等到特定时刻（比如事件发生或异步任务完成）你再来调用它。
