...
- 如果选择了稍后安装操作系统，需要选择设置 ==》 CD/DVD  ==》 使用ISO映像文件 ==》 选择下载好的镜像
...
- 必须安装启动引导器（如：GRUB）
- 启动引导器按照位置 ： /dev/sda
- 安装完成后，关机，需要选择设置 ==》 CD/DVD  ==》 使用物理驱动器
...
- **Linux的层次结构标准（FHS）**
	...
	- Linux 系统的目录组织结构， 遵循了一个文件系统层次结构标准（英语： Filesystem Hierarchy
		...
		- | 目录       | 英文全称         | 主要作用                                                     |
			...
			| **/proc**  | process          | **虚拟文件系统（系统数据）**，记录**内核**和进程信息（ps、top 的数据来源）—— 内核数据结构 |
			...
			| **/sys**   | sysfs            | **虚拟文件系统（系统框架）**，提供**内核设备**、驱动、模块信息 —— 内核驱动模型 |
...
- 报错
  ...
  - cmake编译如果出现 // 注释错误，那么就是gcc编译器工具包，使用到了C89，需要通过`shift + ctrl + p`，输入：`Edit User Local Kits `
  		...
  		[
  		  {
  		    ...
  		    // 加上下面这一行，强制这个 Kit 编译 C 语言时采用 c99/gnu11 标准
  		    "cCompilerFlags": ["-std=gnu11"] 
  		  }
  		]
  		```
...
- | 项目             | **编译型语言**（Compiled Language）                 | **解释型语言**（Interpreted Language）                       |
	...
	| **可移植性**     | 较差（不同平台需重新编译源码）                      | 较好（同一份代码可在不同平台运行）                           |
